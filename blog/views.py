...
class PasiveView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        resistors_potenciometer = Resistors.objects.filter(
            division__contains='potenciometer')
        resistors_trimmer = Resistors.objects.filter(
            division__contains='accuracy')
        resistors_general = Resistors.objects.filter(
            division__contains='general')
        # Capacitors
        ...
        capacitors_e = Capacitors.objects.filter(
            division__contains='electrolitic')
        capacitors_p = Capacitors.objects.filter(
            division__contains='polyester')
        # Diodes
...
class ActiveView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        transistors_g = Transistors.objects.filter(
            division__contains='general')
        transistors_b = Transistors.objects.filter(
            division__contains='bipolar')
        ...
        transistors_p = Transistors.objects.filter(
            division__contains='fototransistor')
